...
def test_evaluation(model, criterion, loader, device, args, mode='test'):
    ...
    # save x and y coordinate(nornalized)
    x_cord_n = []
    y_cord_n = []
    ...
    for (x, x2, y, dates) in loader: 
        ...
        x_cord_n.extend(list(map(float, x[1][:,0,-2])))
        y_cord_n.extend(list(map(float, x[1][:,0,-1])))
    ...
    #change x and y coordinate to before normalized
    mean_std = pkl.load(open(os.path.join( args['res_dir'],'ms_extra.pkl' ), 'rb'))    
    mean_x = mean_std[0][-2]
    mean_y = mean_std[0][-1]
    std_x = mean_std[1][-2]
    std_y = mean_std[1][-1]    
    x_cord = [(x * std_x) + mean_x for x in x_cord_n]
    y_cord = [(y * std_y) + mean_y for y in y_cord_n]
    ...
    return metrics, confusion_matrix(y_true, y_pred, labels=list(range(args['num_classes']))), y_true, y_pred, x_cord, y_cord
...
def get_pse(folder, args):
    ...
    if args['preload']:
        dt = PixelSetData_preloaded(args[folder],pms_extra=args['res_dir'], labels=args['label_class'], npixel=args['npixel'],
                          sub_classes = None,
                          norm_s1=mean_std1,
                          norm_s2=mean_std2,
        ...
        dt = PixelSetData(args[folder],pms_extra=args['res_dir'] , labels=args['label_class'], npixel=args['npixel'],
                          sub_classes = None,
                          norm_s1=mean_std1,
                          norm_s2=mean_std2,
                          minimum_sampling=args['minimum_sampling'],
...
def save_results(metrics, conf_mat, args, y_true, y_pred, x_cord, y_cord):
    ...
    pkl.dump(x_cord, open(os.path.join(args['res_dir'], 'x_cord_test_data.pkl'), 'wb'))
    pkl.dump(y_cord, open(os.path.join(args['res_dir'], 'y_cord_test_data.pkl'), 'wb'))
...
it changed numerical_labels to string_labels 
  ...
  """
  y_pred = os.path.join(args['res_dir'],"y_pred_test_data.pkl")
  x_cord = os.path.join(args['res_dir'],"x_cord_test_data.pkl")
  y_cord = os.path.join(args['res_dir'],"y_cord_test_data.pkl")
...
def main(args):
    ...
    for _, (train_loader, val_loader, test_loader) in enumerate(loaders):
        ...
        test_metrics, conf_mat, y_true, y_pred, x_cord, y_cord = test_evaluation(model, criterion, test_loader, device=device, mode='test', args=args) 

        print('Loss {:.4f},  Acc {:.2f},  IoU {:.4f}'.format(test_metrics['test_loss'], test_metrics['test_accuracy'],
                                                             test_metrics['test_IoU']))
                                                             
        save_results(test_metrics, conf_mat, args, y_true, y_pred,  x_cord, y_cord) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--res_dir', default='./results_convlstm_test', help='Path to the folder where the results should be stored')
